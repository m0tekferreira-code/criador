...
export default function App() {
  ...
  // ========================================================================
  // AGENT 0 -- Carregamento dinamico de modelos
  // ========================================================================
  ...
  // Analisa o manual, logo e referencias visuais.
  // Produz: paleta de cores, analise da marca, conceito de design visual.
  // NAO produz copy -- isso e responsabilidade do Agent 2.
  // ========================================================================
...
Sua especialidade e analise de marca e direcao de arte.

TAREFA:
Analise minuciosamente o manual/documento da marca e TODAS as imagens de referencia visual fornecidas.

ENTREGAVEIS:
1. **brand_analysis** — Uma analise profunda da marca (em Portugues). Inclua:
   ...
   Essa analise sera repassada ao Copywriter para produzir textos de alta conversao.

2. **colors** — Paleta de 3 a 5 cores em formato HEX que representem a marca.
   Baseie-se nas cores do manual, da logo (se enviada) e das referencias visuais.
   As cores devem ser coerentes entre si e adequadas para design publicitario.

3. **design_concept** — Conceito de design visual HIPER DETALHADO, escrito EM INGLES (sera usado como prompt para IA geradora de imagens).
   - Descreva: composicao, background, atmosfera, iluminacao, texturas, estilo fotografico/ilustrativo
   - Baseie-se ESTRITAMENTE no estilo visual das imagens de referencia enviadas
   - Inclua direcao de espaco negativo adequado (geralmente nos cantos) para sobreposicao de logo e texto
   - NAO inclua texto, copy ou tipografia no conceito visual — a copy sera adicionada separadamente
   - O formato final e ${selectedSize} (aspect ratio ${SIZE_TO_ASPECT[selectedSize]})

IMPORTANTE:
- Analise CADA imagem de referencia individualmente e extraia padroes visuais comuns.
- Se uma logo foi enviada, extraia as cores primarias dela para compor a paleta de forma coerente.
- O design_concept deve ser um prompt de imagem profissional, sem mencao a texto ou tipografia.
- Responda APENAS com JSON valido.
      ...
      if (referenceImages.length > 0) {
        parts.push({ text: `=== IMAGENS DE REFERENCIA DE ESTILO (${referenceImages.length} imagem(ns)) — analise o estilo visual de cada uma ===` });
      ...
      const payload = {
        ...
        generationConfig: {
          ...
          responseSchema: {
            type: "OBJECT",
            properties: {
              brand_analysis: { type: "STRING", description: "Analise profunda da marca em Portugues: publico-alvo, tom de voz, proposta de valor, diferenciais, gatilhos emocionais" },
              colors: { type: "ARRAY", items: { type: "STRING" }, description: "Array de 3 a 5 codigos HEX (ex: #FFFFFF)" },
              design_concept: { type: "STRING", description: "Conceito de design hiper detalhado em INGLES para prompt de IA geradora de imagens. Descreva composicao, background, atmosfera, iluminacao, texturas, estilo. SEM texto/tipografia." }
            },
            required: ["brand_analysis", "colors", "design_concept"]
          }
        }
      };
      ...
      // Encadeia automaticamente com o Agente 2 (Copywriter)
  ...
  // Recebe a analise de marca + paleta do Agent 1 e o manual original.
  // Produz: copy persuasiva de alta conversao.
  // Skill: Escritor publicitario senior com dominio de gatilhos mentais,
  // storytelling, frameworks de persuasao (AIDA, PAS, BAB).
  // ========================================================================
...
Voce domina TODOS os frameworks de persuasao: AIDA (Atencao, Interesse, Desejo, Acao), PAS (Problema, Agitacao, Solucao), BAB (Before, After, Bridge), e os 6 principios de Cialdini (Reciprocidade, Escassez, Autoridade, Consistencia, Afinidade, Prova Social).
...
Paleta de cores da campanha: ${colors.join(', ')}
Formato do criativo: ${selectedSize} (${SIZE_TO_ASPECT[selectedSize]})

TAREFA:
Com base no briefing acima E no manual completo da empresa (abaixo), escreva a copy final do anuncio.

DIRETRIZES DE ESCRITA:
1. A copy DEVE ser em Portugues (pt-BR), curta e extremamente impactante.
2. Maximo de 2-3 linhas — precisa caber visualmente em um banner ${selectedSize}.
3. Use gatilhos mentais poderosos (escassez, urgencia, prova social, exclusividade, autoridade — escolha os mais adequados ao contexto).
4. O tom de voz deve ser IDENTICO ao identificado na analise de marca.
5. Foque em alta conversao: o leitor deve sentir compulsao a agir.
6. Inclua um CTA (Call-to-Action) implicito ou explicito quando apropriado.
7. Evite cliches publicitarios genericos ("o melhor", "numero 1", "imperdivel"). Seja original e especifico.
8. Considere o publico-alvo e os gatilhos emocionais identificados no briefing.

PRODUCAO:
- **copy**: O texto final do anuncio, pronto para ser usado no criativo.
- **copy_rationale**: Breve explicacao (1-2 frases) do framework e gatilhos usados, para que o usuario entenda a estrategia.
...
Responda APENAS com JSON valido.
      ...
      const payload = {
        ...
        generationConfig: {
          ...
          responseSchema: {
            type: "OBJECT",
            properties: {
              copy: { type: "STRING", description: "Copy curta, persuasiva e de alta conversao em Portugues (pt-BR)" },
              copy_rationale: { type: "STRING", description: "Breve explicacao da estrategia, framework e gatilhos mentais utilizados" }
            },
  ...
  // ========================================================================
  // AGENT 3 -- Gerador de Imagens
  // Recebe o conceito de design do Agent 1 + paleta.
  // Gera o visual final do anuncio SEM tipografia (a copy sera overlayed).
  // Suporta Imagen (predict) e Gemini Image (generateContent).
  // ========================================================================
  const generateCreative = async () => {
    setStep(4); setLoadingMsg(`Agente 3 — Gerando ${numCreatives} criativo(s) visuais...`); setError(null);
    try {
      const activeKey = customApiKey || apiKey;
      ...
      const promises = Array.from({ length: numCreatives }).map(async (_, index) => {
        const finalPrompt = `
High quality, premium advertising visual composition. Variation ${index + 1} of ${numCreatives} — vary the composition angle and lighting while maintaining the same visual identity.
Aspect ratio: ${aspectRatio}.
Visual Concept: ${designConcept}.
Color Palette that MUST dominate the design: ${palette.join(', ')}.
IMPORTANT: Do NOT include any text, typography, letters, words, or numbers in the image. The image must be purely visual — text will be composited separately in post-production.
Leave clean negative space (approximately 30% of the composition) suitable for text overlay, preferably in the center or lower third.
        `.trim();
        ...
        if (imageModel.startsWith('imagen')) {
          ...
          // Envia referencias visuais para o Gemini Image para style transfer
          if (referenceImages.length > 0) {
            contentParts.push({ text: "Use the following reference images as style guidance:" });
        ...
        <div className="mt-6 p-5 bg-[#f0f7ff] rounded-2xl border border-[#cce4ff] flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-[#0057b3]/90 leading-relaxed mb-3">
              Para ter acesso aos modelos visuais avancados e LLMs pesados (como <b>Gemini 1.5 Pro, Nano Banana Pro e Imagen 4.0</b>), sua Chave API precisa estar vinculada a um projeto com faturamento ativo (Pay-as-you-go). Sem isso, a geracao de imagens falhara.
            </p>
      ...
      <div className="bg-white p-8 rounded-[2rem] shadow-[0_2px_20px_rgba(0,0,0,0.02)] border border-gray-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <p className="text-xs text-[#86868b] mt-2">Diretor de Criacao + Copywriter (2 agentes).</p>
          </div>
          <div>
            ...
            <p className="text-xs text-[#86868b] mt-2">Cria o design visual final do anuncio.</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-8 rounded-[2rem] shadow-[0_2px_20px_rgba(0,0,0,0.02)] border border-gray-100">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-medium text-[#86868b]">Referencias de Estilo</p>
          </div>
  ...
  const renderStep3 = () => (
    <div className="space-y-8 animate-fade-in w-full max-w-4xl mx-auto">
      <div className="text-center mb-10">
        ...
        <p className="text-[#86868b] text-lg">Revise os resultados dos agentes de IA antes de gerar o visual.</p>
      </div>
      ...
      {brandAnalysis && (
        <div className="bg-white p-8 md:p-10 rounded-[2rem] shadow-[0_4px_24px_rgba(0,0,0,0.04)] border border-gray-100">
          <h3 className="text-base font-semibold text-[#1d1d1f] flex items-center gap-2 mb-4 tracking-tight">
            <BookOpen className="w-5 h-5 text-[#86868b]" /> Analise de Marca <span className="text-xs font-normal text-[#86868b] bg-[#f5f5f7] px-2 py-0.5 rounded-full">Agente 1 — Diretor de Criacao</span>
          </h3>
      ...
      <div className="bg-white p-8 md:p-10 rounded-[2rem] shadow-[0_4px_24px_rgba(0,0,0,0.04)] border border-gray-100">
        <div className="space-y-10">
          ...
          <div>
            <h3 className="text-base font-semibold text-[#1d1d1f] flex items-center gap-2 mb-4 tracking-tight">
              <PenTool className="w-5 h-5 text-[#86868b]" /> Copywriting <span className="text-xs font-normal text-[#86868b] bg-[#f5f5f7] px-2 py-0.5 rounded-full">Agente 2 — Copywriter Senior</span>
            </h3>
            {copyRationale && (
          ...
          <div>
            <h3 className="text-base font-semibold text-[#1d1d1f] flex items-center gap-2 mb-4 tracking-tight">
              <Wand2 className="w-5 h-5 text-[#86868b]" /> Direcao de Arte (Prompt Visual) <span className="text-xs font-normal text-[#86868b] bg-[#f5f5f7] px-2 py-0.5 rounded-full">Agente 1</span>
            </h3>
            ...
            <p className="text-xs text-[#86868b] mt-3 ml-2">Mantido em ingles para maximizar a precisao da IA visual. Sem tipografia — a copy sera adicionada separadamente.</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 pt-4">
        ...
        <button onClick={generateCreative}
          ...
          <ImageIcon className="w-5 h-5" /> Gerar Anuncio{numCreatives > 1 ? 's' : ''}
        </button>
      </div>
    </div>
  );
    ...
    return (
      <div className="space-y-10 animate-fade-in w-full max-w-6xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-[#86868b] text-lg">Suas variacoes foram geradas com sucesso.</p>
        </div>
...
export default function App() {
  ...
  const [numCreatives, setNumCreatives] = useState(1);
  const [availableTextModels, setAvailableTextModels] = useState<ModelOption[]>([
    { id: "gemini-2.5-flash-preview-09-2025", label: "Gemini 2.5 Flash (PadrÃ£o)" },
    { id: "gemini-1.5-pro", label: "Gemini 1.5 Pro (Alta Complexidade)" },
    { id: "gemini-1.5-flash", label: "Gemini 1.5 Flash (RÃ¡pido)" }
  ]);
  const [availableImageModels, setAvailableImageModels] = useState<ModelOption[]>([
    { id: "gemini-2.5-flash-image-preview", label: "Gemini Image Flash (Nano Banana)" },
    { id: "gemini-3-pro-image-preview-11-2025", label: "Gemini 3 Pro Image (Nano Banana Pro)" },
    { id: "imagen-4.0-generate-001", label: "Imagen 4.0 (EstÃ¡vel)" }
  ]);
  const [textModel, setTextModel] = useState("gemini-2.5-flash-preview-09-2025");
  const [imageModel, setImageModel] = useState("gemini-2.5-flash-image-preview");
  const [keyStatus, setKeyStatus] = useState<"idle" | "loading" | "success" | "error">("idle");
  ...
  // ========================================================================
  // AGENT 0 â€” Carregamento dinÃ¢mico de modelos
  // ========================================================================
  const fetchModelsFromApi = useCallback(async (key: string) => {
    if (!key || key.length < 10) return;
    setKeyStatus("loading");
    ...
    try {
      ...
      console.error(err);
      setKeyStatus("error");
      setError("Falha ao carregar modelos. Verifique se a chave Ã© vÃ¡lida e possui permissÃµes.");
    }
  }, []);
  ...
  // ========================================================================
  // AGENT 1 â€” Diretor de CriaÃ§Ã£o (Analista de Marca + DireÃ§Ã£o de Arte)
  // Analisa o manual, logo e referÃªncias visuais.
  // Produz: paleta de cores, anÃ¡lise da marca, conceito de design visual.
  // NÃƒO produz copy â€” isso Ã© responsabilidade do Agent 2.
  // ========================================================================
  const analyzeInputs = async () => {
    if (!mdContent) { setError("Por favor, faÃ§a upload do arquivo Markdown com as informaÃ§Ãµes da empresa."); return; }
    setStep(2); setLoadingMsg("Agente 1 â€” Analisando marca e direÃ§Ã£o de arte..."); setError(null);
    try {
      const activeKey = customApiKey || apiKey;
      const url = `https://generativelanguage.googleapis.com/v1beta/models/${textModel}:generateContent?key=${activeKey}`;
      const systemPrompt = `
VocÃª Ã© um Diretor de CriaÃ§Ã£o SÃªnior em uma agÃªncia de publicidade de elite com mais de 20 anos de experiÃªncia.
Sua especialidade Ã© anÃ¡lise de marca e direÃ§Ã£o de arte.

TAREFA:
Analise minuciosamente o manual/documento da marca e TODAS as imagens de referÃªncia visual fornecidas.

ENTREGÃVEIS:
1. **brand_analysis** â€” Uma anÃ¡lise profunda da marca (em PortuguÃªs). Inclua:
   - PÃºblico-alvo e persona identificados
   - Tom de voz e personalidade da marca
   - Proposta de valor principal
   - Diferenciais competitivos
   - Gatilhos emocionais que ressoam com o pÃºblico
   Essa anÃ¡lise serÃ¡ repassada ao Copywriter para produzir textos de alta conversÃ£o.

2. **colors** â€” Paleta de 3 a 5 cores em formato HEX que representem a marca.
   Baseie-se nas cores do manual, da logo (se enviada) e das referÃªncias visuais.
   As cores devem ser coerentes entre si e adequadas para design publicitÃ¡rio.

3. **design_concept** â€” Conceito de design visual HIPER DETALHADO, escrito EM INGLÃŠS (serÃ¡ usado como prompt para IA geradora de imagens).
   - Descreva: composiÃ§Ã£o, background, atmosfera, iluminaÃ§Ã£o, texturas, estilo fotogrÃ¡fico/ilustrativo
   - Baseie-se ESTRITAMENTE no estilo visual das imagens de referÃªncia enviadas
   - Inclua direÃ§Ã£o de espaÃ§o negativo adequado (geralmente nos cantos) para sobreposiÃ§Ã£o de logo e texto
   - NÃƒO inclua texto, copy ou tipografia no conceito visual â€” a copy serÃ¡ adicionada separadamente
   - O formato final Ã© ${selectedSize} (aspect ratio ${SIZE_TO_ASPECT[selectedSize]})

IMPORTANTE:
- Analise CADA imagem de referÃªncia individualmente e extraia padrÃµes visuais comuns.
- Se uma logo foi enviada, extraia as cores primÃ¡rias dela para compor a paleta de forma coerente.
- O design_concept deve ser um prompt de imagem profissional, sem menÃ§Ã£o a texto ou tipografia.
- Responda APENAS com JSON vÃ¡lido.
      ...
      const parts: any[] = [{ text: `=== MANUAL DA EMPRESA ===\n${mdContent}` }];
      if (logo) {
        parts.push({ text: "=== LOGO DA EMPRESA (extraia as cores primÃ¡rias) ===" });
        parts.push({ inlineData: { mimeType: "image/png", data: logo.split(',')[1] } });
      }
      if (referenceImages.length > 0) {
        parts.push({ text: `=== IMAGENS DE REFERÃŠNCIA DE ESTILO (${referenceImages.length} imagem(ns)) â€” analise o estilo visual de cada uma ===` });
        referenceImages.forEach((img, i) => {
          parts.push({ text: `ReferÃªncia visual #${i + 1}:` });
          parts.push({ inlineData: { mimeType: img.mimeType, data: img.base64.split(',')[1] } });
        });
      }
      ...
      const payload = {
        ...
        generationConfig: {
          ...
          responseSchema: {
            type: "OBJECT",
            properties: {
              brand_analysis: { type: "STRING", description: "AnÃ¡lise profunda da marca em PortuguÃªs: pÃºblico-alvo, tom de voz, proposta de valor, diferenciais, gatilhos emocionais" },
              colors: { type: "ARRAY", items: { type: "STRING" }, description: "Array de 3 a 5 cÃ³digos HEX (ex: #FFFFFF)" },
              design_concept: { type: "STRING", description: "Conceito de design hiper detalhado em INGLÃŠS para prompt de IA geradora de imagens. Descreva composiÃ§Ã£o, background, atmosfera, iluminaÃ§Ã£o, texturas, estilo. SEM texto/tipografia." }
            },
            required: ["brand_analysis", "colors", "design_concept"]
          }
        }
      ...
      const result = await fetchWithRetry(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      const responseText = result.candidates?.[0]?.content?.parts?.[0]?.text;
      if (!responseText) throw new Error("Resposta invÃ¡lida da IA analÃ­tica.");
      const data = JSON.parse(responseText);
      ...
      console.error(err); setError("Erro no Agente AnalÃ­tico. " + err.message); setStep(1);
    }
  };

  // ========================================================================
  // AGENT 2 â€” Copywriter SÃªnior
  // Recebe a anÃ¡lise de marca + paleta do Agent 1 e o manual original.
  // Produz: copy persuasiva de alta conversÃ£o.
  // Skill: Escritor publicitÃ¡rio sÃªnior com domÃ­nio de gatilhos mentais,
  // storytelling, frameworks de persuasÃ£o (AIDA, PAS, BAB).
  // ========================================================================
  const generateCopy = async (analysis: string, colors: string[]) => {
    setLoadingMsg("Agente 2 â€” Copywriter escrevendo textos de alta conversÃ£o...");
    try {
      const activeKey = customApiKey || apiKey;
      const url = `https://generativelanguage.googleapis.com/v1beta/models/${textModel}:generateContent?key=${activeKey}`;

      const systemPrompt = `
VocÃª Ã© um Copywriter SÃªnior de elite com mais de 15 anos de experiÃªncia em campanhas publicitÃ¡rias de alta performance.
VocÃª domina TODOS os frameworks de persuasÃ£o: AIDA (AtenÃ§Ã£o, Interesse, Desejo, AÃ§Ã£o), PAS (Problema, AgitaÃ§Ã£o, SoluÃ§Ã£o), BAB (Before, After, Bridge), e os 6 princÃ­pios de Cialdini (Reciprocidade, Escassez, Autoridade, ConsistÃªncia, Afinidade, Prova Social).

CONTEXTO:
Um Diretor de CriaÃ§Ã£o analisou a marca e produziu o seguinte briefing:
...
TAREFA:
Com base no briefing acima E no manual completo da empresa (abaixo), escreva a copy final do anÃºncio.

DIRETRIZES DE ESCRITA:
1. A copy DEVE ser em PortuguÃªs (pt-BR), curta e extremamente impactante.
2. MÃ¡ximo de 2-3 linhas â€” precisa caber visualmente em um banner ${selectedSize}.
3. Use gatilhos mentais poderosos (escassez, urgÃªncia, prova social, exclusividade, autoridade â€” escolha os mais adequados ao contexto).
4. O tom de voz deve ser IDÃŠNTICO ao identificado na anÃ¡lise de marca.
5. Foque em alta conversÃ£o: o leitor deve sentir compulsÃ£o a agir.
6. Inclua um CTA (Call-to-Action) implÃ­cito ou explÃ­cito quando apropriado.
7. Evite clichÃªs publicitÃ¡rios genÃ©ricos ("o melhor", "nÃºmero 1", "imperdÃ­vel"). Seja original e especÃ­fico.
8. Considere o pÃºblico-alvo e os gatilhos emocionais identificados no briefing.

PRODUÃ‡ÃƒO:
- **copy**: O texto final do anÃºncio, pronto para ser usado no criativo.
- **copy_rationale**: Breve explicaÃ§Ã£o (1-2 frases) do framework e gatilhos usados, para que o usuÃ¡rio entenda a estratÃ©gia.
...
Responda APENAS com JSON vÃ¡lido.
      ...
      const payload = {
        ...
        generationConfig: {
          ...
          responseSchema: {
            type: "OBJECT",
            properties: {
              copy: { type: "STRING", description: "Copy curta, persuasiva e de alta conversÃ£o em PortuguÃªs (pt-BR)" },
              copy_rationale: { type: "STRING", description: "Breve explicaÃ§Ã£o da estratÃ©gia, framework e gatilhos mentais utilizados" }
            },
            required: ["copy", "copy_rationale"]
          }
        }
      ...
      const result = await fetchWithRetry(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      const responseText = result.candidates?.[0]?.content?.parts?.[0]?.text;
      if (!responseText) throw new Error("Resposta invÃ¡lida do Copywriter.");
      const data = JSON.parse(responseText);
  ...
  // ========================================================================
  // AGENT 3 â€” Gerador de Imagens
  // Recebe o conceito de design do Agent 1 + paleta.
  // Gera o visual final do anÃºncio SEM tipografia (a copy serÃ¡ overlayed).
  // Suporta Imagen (predict) e Gemini Image (generateContent).
  // ========================================================================
  const generateCreative = async () => {
    setStep(4); setLoadingMsg(`Agente 3 â€” Gerando ${numCreatives} criativo(s) visuais...`); setError(null);
    try {
      const activeKey = customApiKey || apiKey;
      const aspectRatio = SIZE_TO_ASPECT[selectedSize] || "1:1";
      ...
      const promises = Array.from({ length: numCreatives }).map(async (_, index) => {
        const finalPrompt = `
High quality, premium advertising visual composition. Variation ${index + 1} of ${numCreatives} â€” vary the composition angle and lighting while maintaining the same visual identity.
Aspect ratio: ${aspectRatio}.
Visual Concept: ${designConcept}.
Color Palette that MUST dominate the design: ${palette.join(', ')}.
IMPORTANT: Do NOT include any text, typography, letters, words, or numbers in the image. The image must be purely visual â€” text will be composited separately in post-production.
Leave clean negative space (approximately 30% of the composition) suitable for text overlay, preferably in the center or lower third.
        `.trim();
        ...
        if (imageModel.startsWith('imagen')) {
          ...
          // Envia referÃªncias visuais para o Gemini Image para style transfer
          if (referenceImages.length > 0) {
            contentParts.push({ text: "Use the following reference images as style guidance:" });
            referenceImages.forEach((img) => {
              contentParts.push({ inlineData: { mimeType: img.mimeType, data: img.base64.split(',')[1] } });
  ...
  const renderStep1 = () => (
    <div className="space-y-8 animate-fade-in w-full max-w-3xl mx-auto">
      <div className="bg-white p-8 rounded-[2rem] shadow-[0_2px_20px_rgba(0,0,0,0.02)] border border-gray-100">
        <h3 className="text-xl font-semibold text-[#1d1d1f] mb-2 tracking-tight">ConfiguraÃ§Ã£o de API e Modelos</h3>
        <p className="text-sm text-[#86868b] mb-5">Insira sua chave do Google AI Studio â€” os modelos serÃ£o carregados automaticamente.</p>
        <div className="relative">
          <Key className="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
          <input type="password" placeholder="AIzaSy..." value={customApiKey}
            onChange={(e) => setCustomApiKey(e.target.value)}
        ...
        {keyStatus === "error" && (
          <p className="text-sm text-red-500 font-medium mt-3 flex items-center gap-1.5 ml-1">
            <AlertCircle className="w-4 h-4" /> Chave invÃ¡lida ou sem permissÃ£o. Verifique e tente novamente.
          </p>
        )}
        <div className="mt-6 p-5 bg-[#f0f7ff] rounded-2xl border border-[#cce4ff] flex items-start gap-4">
          <Info className="w-6 h-6 text-[#0071e3] shrink-0 mt-0.5" />
          <div>
            <p className="font-semibold text-[#004a99] mb-1">Requisito de Faturamento (NÃ­vel 1)</p>
            <p className="text-sm text-[#0057b3]/90 leading-relaxed mb-3">
              Para ter acesso aos modelos visuais avanÃ§ados e LLMs pesados (como <b>Gemini 1.5 Pro, Nano Banana Pro e Imagen 4.0</b>), sua Chave API precisa estar vinculada a um projeto com faturamento ativo (Pay-as-you-go). Sem isso, a geraÃ§Ã£o de imagens falharÃ¡.
            </p>
            <a href="https://aistudio.google.com/app/billing" target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 text-[#0071e3] hover:text-[#0057b3] font-semibold text-sm transition-colors">
              Ativar faturamento no AI Studio <ExternalLink className="w-4 h-4" />
      ...
      <div className="bg-white p-8 rounded-[2rem] shadow-[0_2px_20px_rgba(0,0,0,0.02)] border border-gray-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <p className="text-sm font-medium text-[#86868b] mb-3">IA AnalÃ­tica e Copywriter</p>
            <div className="relative">
              <select value={textModel} onChange={(e) => setTextModel(e.target.value)}
                className="w-full p-4 pr-12 bg-[#f5f5f7] border-transparent rounded-2xl outline-none focus:bg-white focus:border-gray-200 focus:ring-4 focus:ring-gray-100 text-[#1d1d1f] font-medium text-sm transition-all appearance-none cursor-pointer">
                {availableTextModels.map(model => (<option key={model.id} value={model.id}>{model.label}</option>))}
            ...
            <p className="text-xs text-[#86868b] mt-2">Diretor de CriaÃ§Ã£o + Copywriter (2 agentes).</p>
          </div>
          <div>
            <p className="text-sm font-medium text-[#86868b] mb-3">IA Geradora de Imagens</p>
            <div className="relative">
            ...
            <p className="text-xs text-[#86868b] mt-2">Cria o design visual final do anÃºncio.</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-8 rounded-[2rem] shadow-[0_2px_20px_rgba(0,0,0,0.02)] border border-gray-100">
        ...
        {!mdFile ? (
          <div onClick={() => mdInputRef.current?.click()}
            ...
            <UploadCloud className="w-10 h-10 text-[#86868b] mx-auto mb-4" strokeWidth={1.5} />
            <p className="text-[#1d1d1f] font-medium text-lg">Carregar documento de marca</p>
            <p className="text-[#86868b] text-sm mt-2">Tom de voz, missÃ£o, produto, etc.</p>
          </div>
        ) : (
          <div className="flex items-center justify-between bg-[#f5f5f7] p-5 rounded-2xl">
            <div className="flex items-center gap-4">
      ...
      <div className="bg-white p-8 rounded-[2rem] shadow-[0_2px_20px_rgba(0,0,0,0.02)] border border-gray-100">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-medium text-[#86868b]">ReferÃªncias de Estilo</p>
          </div>
          <input type="file" accept="image/*" multiple ref={imageInputRef} className="hidden" onChange={handleImageUpload} />
          <div className="grid grid-cols-3 md:grid-cols-4 gap-4">
            {referenceImages.map((img, idx) => (
              <div key={idx} className="relative aspect-square group rounded-2xl overflow-hidden shadow-sm">
                <img src={img.base64} alt={`Ref ${idx}`} className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity" />
                <button onClick={() => removeImage(idx)} title="Remover referÃªncia"
                  className="absolute top-2 right-2 p-1.5 bg-white/90 backdrop-blur-md rounded-full text-[#1d1d1f] opacity-0 group-hover:opacity-100 transition-all hover:scale-105">
                  <X className="w-4 h-4" />
                </button>
              </div>
      ...
      <div className="bg-white p-8 rounded-[2rem] shadow-[0_2px_20px_rgba(0,0,0,0.02)] border border-gray-100">
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <span className="text-sm font-medium text-[#86868b]">Quantidade de VariaÃ§Ãµes</span>
            <span className="text-lg font-semibold text-[#1d1d1f] bg-[#f5f5f7] px-4 py-1 rounded-full">{numCreatives}</span>
          </div>
          <input type="range" min="1" max="10" value={numCreatives}
            onChange={(e) => setNumCreatives(Number(e.target.value))}
  ...
  const renderStep3 = () => (
    <div className="space-y-8 animate-fade-in w-full max-w-4xl mx-auto">
      <div className="text-center mb-10">
        <h2 className="text-3xl font-semibold text-[#1d1d1f] tracking-tight mb-3">AprovaÃ§Ã£o Criativa</h2>
        <p className="text-[#86868b] text-lg">Revise os resultados dos agentes de IA antes de gerar o visual.</p>
      </div>

      {/* AnÃ¡lise de Marca (Agent 1) */}
      {brandAnalysis && (
        <div className="bg-white p-8 md:p-10 rounded-[2rem] shadow-[0_4px_24px_rgba(0,0,0,0.04)] border border-gray-100">
          <h3 className="text-base font-semibold text-[#1d1d1f] flex items-center gap-2 mb-4 tracking-tight">
            <BookOpen className="w-5 h-5 text-[#86868b]" /> AnÃ¡lise de Marca <span className="text-xs font-normal text-[#86868b] bg-[#f5f5f7] px-2 py-0.5 rounded-full">Agente 1 â€” Diretor de CriaÃ§Ã£o</span>
          </h3>
          <div className="p-5 bg-[#f5f5f7] rounded-2xl text-sm text-[#1d1d1f] leading-relaxed whitespace-pre-wrap max-h-60 overflow-y-auto">
            {brandAnalysis}
          </div>
      ...
      <div className="bg-white p-8 md:p-10 rounded-[2rem] shadow-[0_4px_24px_rgba(0,0,0,0.04)] border border-gray-100">
        <div className="space-y-10">
          ...
          <div>
            <h3 className="text-base font-semibold text-[#1d1d1f] flex items-center gap-2 mb-4 tracking-tight">
              <PenTool className="w-5 h-5 text-[#86868b]" /> Copywriting <span className="text-xs font-normal text-[#86868b] bg-[#f5f5f7] px-2 py-0.5 rounded-full">Agente 2 â€” Copywriter SÃªnior</span>
            </h3>
            {copyRationale && (
              <div className="mb-4 p-4 bg-amber-50/60 border border-amber-100 rounded-2xl flex items-start gap-3">
                <Info className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
          ...
          {/* DireÃ§Ã£o de Arte (Agent 1) */}
          <div>
            <h3 className="text-base font-semibold text-[#1d1d1f] flex items-center gap-2 mb-4 tracking-tight">
              <Wand2 className="w-5 h-5 text-[#86868b]" /> DireÃ§Ã£o de Arte (Prompt Visual) <span className="text-xs font-normal text-[#86868b] bg-[#f5f5f7] px-2 py-0.5 rounded-full">Agente 1</span>
            </h3>
            <textarea value={designConcept} onChange={(e) => setDesignConcept(e.target.value)}
              className="w-full p-5 bg-[#f5f5f7] border-transparent rounded-2xl text-[#86868b] text-sm resize-none focus:bg-white focus:border-gray-200 focus:ring-4 focus:ring-gray-100 outline-none h-40 font-mono transition-all leading-relaxed" />
            <p className="text-xs text-[#86868b] mt-3 ml-2">Mantido em inglÃªs para maximizar a precisÃ£o da IA visual. Sem tipografia â€” a copy serÃ¡ adicionada separadamente.</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 pt-4">
        ...
        <button onClick={generateCreative}
          className="flex-1 py-4 bg-[#0071e3] hover:bg-[#0077ed] text-white font-semibold text-lg rounded-full flex items-center justify-center gap-2 transition-all shadow-[0_4px_14px_rgba(0,113,227,0.3)] hover:shadow-[0_6px_20px_rgba(0,113,227,0.4)]">
          <ImageIcon className="w-5 h-5" /> Gerar AnÃºncio{numCreatives > 1 ? 's' : ''}
        </button>
      </div>
    </div>
  );
  ...
  const renderStep5 = () => {
    ...
    return (
      <div className="space-y-10 animate-fade-in w-full max-w-6xl mx-auto">
        <div className="text-center mb-8">
          <h2 className="text-3xl md:text-4xl font-semibold text-[#1d1d1f] tracking-tight mb-3">Criativos Prontos</h2>
          <p className="text-[#86868b] text-lg">Suas variaÃ§Ãµes foram geradas com sucesso.</p>
        </div>
        <div className={`grid gap-8 ${generatedImages.length === 1 ? 'grid-cols-1 max-w-xl mx-auto' : selectedSize === "1080x1920" ? 'grid-cols-2 lg:grid-cols-4' : 'grid-cols-1 md:grid-cols-2'}`}>
          {generatedImages.map((img, idx) => (
            <div key={idx} className="flex flex-col">
    ...
    <div className="min-h-screen bg-[#fbfbfd] text-[#1d1d1f] font-sans selection:bg-[#0071e3] selection:text-white p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        <header className="mb-12 text-center pt-8">
          ...
          <p className="text-[#86868b] text-lg font-medium tracking-tight">InteligÃªncia Artificial para criaÃ§Ã£o publicitÃ¡ria.</p>
        </header>
        <div className="flex justify-center mb-16">
          <div className="inline-flex bg-[#f5f5f7] p-1.5 rounded-full border border-gray-100">
            {[{ n: 1, label: "Setup" }, { n: 3, label: "AprovaÃ§Ã£o" }, { n: 5, label: "Resultado" }].map((s, i) => (
              <div key={s.n}
                className={`flex items-center gap-2 px-6 py-2 rounded-full text-sm font-semibold transition-all duration-500 ${step >= s.n ? 'bg-white text-[#1d1d1f] shadow-[0_2px_8px_rgba(0,0,0,0.06)]' : 'text-[#86868b]'}`}>
                <span className={`w-5 h-5 flex items-center justify-center rounded-full text-xs ${step >= s.n ? 'bg-[#1d1d1f] text-white' : 'bg-transparent border border-[#86868b]'}`}>{i + 1}</span>
                {s.label}
